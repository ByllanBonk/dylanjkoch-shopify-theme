...
3. **Email and SMS capture**: every capture surface (the Playbook landing hero, the capture sections, the homepage email forms) shares one form layer. Pick the platform once under Theme settings, Dylan Koch brand, Lead capture platform. **Klaviyo direct** posts email, first name, phone, and SMS consent straight to a Klaviyo list: paste the public API key (Klaviyo, Settings, API keys) and the list ID (Lists and segments, open the list, Settings). If either ID is missing the forms fall back to Shopify customers so no lead is lost. SMS consent comes from the consent checkbox, so leads who tick it and give a phone number are SMS subscribed in Klaviyo too. Custom mode in each section still posts to any other platform's form endpoint (MailerLite, ConvertKit, ActiveCampaign, HubSpot, GoHighLevel, custom APIs). Switching platforms never requires rebuilding the page.
